...
class LitModel(pl.LightningModule):
    def __init__(self, conf: TrainConfig):
        ...
        if os.path.exists(checkpoint_path):
            ...
            self.ckpt = torch.load(resume, map_location=lambda storage, loc: storage)
    ...
    #
    #     # 加载预训练模型的状态字典并调整键名以匹配当前模型的键
    #     pretrained_dict = checkpoint['state_dict']
    #
    #     # 调整键名，删除前缀以匹配当前模型的结构
    #     adjusted_model_dict = {k.replace('model.', ''): v for k, v in pretrained_dict.items() if 'model.' in k}
    #     adjusted_ema_model_dict = {k.replace('ema_model.', ''): v for k, v in pretrained_dict.items() if
    #                                'ema_model.' in k}
    #
    #     # 更新当前模型的权重
    #     updated_model_dict = {k: v for k, v in adjusted_model_dict.items() if k in model_dict}
    #     updated_ema_model_dict = {k: v for k, v in adjusted_ema_model_dict.items() if k in ema_model_dict}
    #
    #     model_dict.update(updated_model_dict)
    #     ema_model_dict.update(updated_ema_model_dict)
    #
    #     # 使用更新后的字典加载到模型中
    #     self.model.load_state_dict(model_dict, strict=False)
    #     self.ema_model.load_state_dict(ema_model_dict, strict=False)
    #
    #     # 初始化未匹配的键
    #     for name, param in self.model.named_parameters():
    #         if name not in updated_model_dict:
    ...
    #         if name not in updated_ema_model_dict:
    #             print(f"Initializing {name}")
    #             if 'weight' in name:
    #                 torch.nn.init.xavier_uniform_(param.data)
    #             elif 'bias' in name:
    ...
    #     # 加载预训练模型的状态字典并调整键名以匹配当前模型的键
    #     pretrained_dict = checkpoint['state_dict']
    #     # 去除前缀，这里假设所有预训练键都以'model.'开始
    #     adjusted_pretrained_dict = {k.replace('model.', ''): v for k, v in pretrained_dict.items()}
    #     adjusted_ema_model_dict = {k.replace('ema_model.', ''): v for k, v in pretrained_dict.items() if
    #                                'ema_model.' in k}
    #     matched_pretrained_dict = {k: v for k, v in adjusted_pretrained_dict.items() if
    #                                    k in model_dict and model_dict[k].size() == v.size()}
    #
    #     init_params = []
    #     for name, param in self.model.named_parameters():
    #         if name not in matched_pretrained_dict:
    #             print(f"Initializing {name}")
    #             if 'weight' in name:
    #                 init.xavier_uniform_(param.data)
    #             elif 'bias' in name:
    #                 init.zeros_(param.data)
    #             checkpoint['state_dict']["model."+name] = param.data
    #             checkpoint['state_dict']["ema_model." + name] = param.data
    #             init_params.append(param)
    #             # checkpoint['optimizer_states']["model."+name] =
    #             # checkpoint['optimizer_states']["ema_model." + name] =
    #     if init_params:
    #         optimizer = torch.optim.Adam(init_params, lr=0.001)
    #         optimizer_state_dict = optimizer.state_dict()
    #
    #         # 检查是否已存在 'optimizer_states' 键
    #         if 'optimizer_states' in checkpoint:
    #             # 如果存在，则追加新的优化器状态
    #             checkpoint['optimizer_states'].append(optimizer_state_dict)
    #
    #         else:
    #             # 如果不存在，则创建新列表并添加优化器状态
    #             checkpoint['optimizer_states'] = [optimizer_state_dict]
    #
    #     else:
    #         print("No parameters needed for special initialization.")
    ...
    def encode(self, x,t):
        ...
        cond = self.ema_model.encoder.forward(x,t)
        return cond
    ...
    def test_step(self, batch, *args, **kwargs):
        ...
        for each in self.conf.eval_programs:
            if each.startswith('fid'):
                ...
                if not os.path.exists(self.conf.logdir):
                    os.makedirs(self.conf.logdir)
                with open(os.path.join(self.conf.logdir, 'FID.txt'),
                          'a') as f:
                    metrics = {
                        f'FID{T}': score,
                        'num_samples': self.num_samples,
...
def train(conf: TrainConfig, gpus, nodes=1, mode: str = 'train'):
    ...
    # conf.save_every_samples  = 1e5 // conf.batch_size_effective = 128 == 781
    ...
    #     # 使用 DDPStrategy 并关闭 find_unused_parameters，适用于梯度检查点优化
    ...
    if mode == 'step1':
        ...
        # optimizer_new = torch.optim.Adam([model.encoder.fc_a.parameters(),
        #                                   model.encoder.fc_mu.parameters(),
        #                                   model.encoder.fc_var.parameters()
        #                                   ], lr=0.001)
        # optimizer_old = checkpoint['optimizer_states']
        ...
        eval_path = 'checkpoints/Bedroom128_ckpt/last.ckpt' #epoch=155-step=85000.ckpt
        #eval_path = conf.wzk_ckpt
